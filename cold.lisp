...
(defmethod glut:display-window :before ((w shader-vao-window))
  ...
  (let ((arr (gl:alloc-gl-array :float 12))
    ...
    (gl:buffer-data :array-buffer (vertex-buffer w))
    (gl:free-gl-array arr))

  ;; 0 is always reserved as an unbound object.
  (gl:bind-buffer :array-buffer 0)

  ;; An element array buffer stores vertex indices, we fill it
  ;; in the same was as an array buffer.
  ...
  ;; which vertex attributes we want to associate it with, and specify
  ...
  ;; To associate an element array with VAO, we all need to do is
  ;; bind the element array buffer we want to use.
  (gl:bind-buffer :element-array-buffer (index-buffer w))

  ;; Once, we are done, we can unbind the VAO, and rebind it when
  ;; We want to render it.
  (gl:bind-vertex-array 0)

  ;; A program object is a collection of shader objects to be used
  ;; together in a single pipeline for rendering objects. To create
  ;; a program, you first create the individual shaders. Then you
  ;; attach the shaders to the program and link the program together.
  (let ((vs (gl:create-shader :vertex-shader))
    ...
    ;; shaders. This step actually puts the attached shader
    ;; together to form the program.
    (gl:link-program (program w))

    ;; If we want to render using this program object, or add
    ;; uniform, we need to use the progrm. This is similar to binding
    ;; a buffer.
